...
function getDateForKey(key, days = -8) {
	const now = new Date();
	switch (key) {
		case 'yesterday':
...
async function main() {
	...
	try {
		...
		// Calculate the target date based on DAY_OFFSET
		const targetDate = new Date();
		targetDate.setDate(targetDate.getDate() + dayOffset);
		const formattedTargetDate = formatDate(targetDate);

		// Fetch and process data for the target date
		console.log(`Processing data for date: ${formattedTargetDate}`);

...
const table = base("01 Beta"); // Updated table name
	...
	events.forEach(event => {
		...
		if (homeTeam) {
			records.push({
				...
				'Team Score': parsedHomeTeamScore,
				'Team Abbreviation': homeTeam.team.abbreviation,
				'Team Logo': homeTeam.team.logo,
				'Team Logo PNG': [{ url: homeTeam.team.logo }], // New field for attachment
				'Home Away': 'Home',
				...
				'Opponent Score': parsedAwayTeamScore
			});
		}
		...
		if (awayTeam) {
			records.push({
				...
				'Team Score': parsedAwayTeamScore,
				'Team Abbreviation': awayTeam.team.abbreviation,
				'Team Logo': awayTeam.team.logo,
				'Team Logo PNG': [{ url: awayTeam.team.logo }], // New field for attachment
				'Home Away': 'Away',
				'Odds': odds ? odds.details : "",
				'Broadcast': broadcastText,
				'Status ID': event.status && event.status.type && event.status.type.id ? event.status.type.id : "",
				'League': 'NBA',
				'Event Time': event.date,
				'Opponent Score': parsedHomeTeamScore
			});
		}
	});
